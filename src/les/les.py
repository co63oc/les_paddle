...
class Les(paddle.nn.Module):
    ...
    def __init__(self, les_arguments: Union[Dict[str, Any], str] = {}):
        ...
        self.atomwise: paddle.nn.Module = (
            Atomwise(
        ...
        self.bec = BEC(remove_mean=self.remove_mean, epsilon_factor=self.epsilon_factor)
    ...
    def forward(
        ...
        cell: paddle.Tensor,
        desc: Optional[paddle.Tensor] = None,
        latent_charges: Optional[paddle.Tensor] = None,
        ...
        if batch is None:
            batch = paddle.zeros(
                positions.shape[0], dtype=paddle.int64, device=positions.device
            )
        if latent_charges is not None:
            ...
            if not self.use_atomwise:
                raise ValueError(
                    "desc must be provided and use_atomwise must be True if latent_charges is not provided"
                )
        ...
        if compute_energy:
            E_lr = self.ewald(q=latent_charges, r=positions, cell=cell, batch=batch)
        else:
            E_lr = None
        if compute_bec:
            bec = self.bec(
            ...
            bec = None
        output = {"E_lr": E_lr, "latent_charges": latent_charges, "BEC": bec}
        return output


class _DummyAtomwise(paddle.nn.Module):

    def forward(self, desc: paddle.Tensor, batch: paddle.Tensor) -> paddle.Tensor:
